    ...
    try {
        ...
        if (postorter.length === 0) {
            ...
            postorter.slice(0, 5).forEach((p) => {
                console.log(`  ${p.post_ort} - ${p.post_nummer}: ${p.foretag_count} personer`);
            });
        }
        ...
        for (const postort of postorter) {
            try {
                ...
                const [rows] = await connection.execute(
                    'SELECT id, foretag_count FROM ratsit_postorter WHERE post_ort = ? AND post_nummer = ?',
                ...
                if (rows.length > 0) {
                    ...
                    await connection.execute(
                        'UPDATE ratsit_postorter SET foretag_count = ?, foretag_link = ?, updated_at = NOW() WHERE id = ?',
                        [
                            postort.foretag_count,
                            postort.foretag_link,
                            rows[0].id,
                        ],
                    );
                    updatedCount++;
                    console.log(`  Updated: ${postort.post_ort} - ${postort.post_nummer} (${postort.foretag_count} personer)`);
                } else {
                    // Insert new record
                    await connection.execute(
                        'INSERT INTO ratsit_postorter (post_ort, post_nummer, foretag_count, foretag_link, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())',
                        [
                            postort.post_ort,
                            postort.post_nummer,
                            postort.foretag_count,
                    ...
                    savedCount++;
                    console.log(`  Saved: ${postort.post_ort} - ${postort.post_nummer} (${postort.foretag_count} personer)`);
                }
            } catch (error) {
