    ...
    public function createAction(): Action
    {
        return Action::make('create')
            ...
            ->fillForm(function (array $arguments) {
                ...
                if (! isset($merged['booking_user_id']) || empty($merged['booking_user_id'])) {
                    $merged['booking_user_id'] = Auth::id();
                }

                // Preserve service_user_id from data if provided (from calendar context)
                if (! isset($merged['service_user_id']) || empty($merged['service_user_id'])) {
                    // If current user is a service person, default to them
                    if ($user && ($user->role === 'service' || (isset($user->role->value) && $user->role->value === 'service'))) {
