...
final class ImportCarryDataCommand extends Command
{
    /**
    ...
    public function handle(): int
    {
        ...
        while (($data = fgetcsv($handle)) !== false) {
            ...
            $batch[] = [
                'person_lopnr' => mb_trim($data[0] ?? '') ?: null,
                'personnr' => mb_trim($data[1] ?? '') ?: null,
                'kon' => mb_trim($data[2] ?? '') ?: null,
                'civilstand' => mb_trim($data[3] ?? '') ?: null,
                'namn' => mb_trim($data[4] ?? '') ?: null,
                'fornamn' => mb_trim($data[5] ?? '') ?: null,
                'efternamn' => mb_trim($data[6] ?? '') ?: null,
                'adress' => mb_trim($data[7] ?? '') ?: null,
                'co_adress' => mb_trim($data[8] ?? '') ?: null,
                'postnr' => mb_trim($data[9] ?? '') ?: null,
                'ort' => mb_trim($data[10] ?? '') ?: null,
                'telefon' => mb_trim($data[11] ?? '') ?: null,
                'mobiltelefon' => mb_trim($data[12] ?? '') ?: null,
                'telefax' => mb_trim($data[13] ?? '') ?: null,
                'epost' => mb_trim($data[14] ?? '') ?: null,
                'epost_privat' => mb_trim($data[15] ?? '') ?: null,
                'epost_sekundar' => mb_trim($data[16] ?? '') ?: null,
                'created_at' => now(),
                'updated_at' => now(),
            ];
