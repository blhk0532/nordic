 ...
 *
 * Saves to: hitta_se, personer_data, and hitta_data tables
 *
 * Features:
 ...
 * - Batch API endpoints: /api/hitta-se/batch, /api/personer-data/bulk, /api/hitta-data/bulk
 */
...
// --- API Configuration ---
const API_BASE = process.env.API_BASE || process.env.APP_URL || 'https://nordicdigitalthailand.com' ||  'https://nordicdigitalthailand.com';
...
async function savePersonsViaApi(persons) {
  ...
  for (let i = 0; i < persons.length; i += batchSize) {
    ...
    try {
      ...
      // DISABLED: PersonerData saves are disabled - do not save to personer_data table
      /*
      ...
      */
      ...
      // Then save to hitta_data table
      try {
        // Create payload for hitta_data with appropriate field mapping
        const hittaDataPayload = {
          records: batch.map((p, index) => {
            const personPayload = mapPersonToApiPayload(p);
            // For hitta_data, map telefon array to both telefon (string) and telefonnumer (array)
        ...
        if (hittaDataErr.response?.data) {
          console.log(`   📋 hitta_data error details: ${JSON.stringify(hittaDataErr.response.data).substring(0, 150)}`);
        }
      }
    } catch (err) {
      totalFailed += batch.length;
  ...
  if (totalFailed > 0 || personerDataFailed > 0 || hittaDataFailed > 0) {
