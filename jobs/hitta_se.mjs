 ...
 * Scrapes person data from hitta.se only
 * Saves Hitta data to `hitta_se` table
 * Upserts combined data to `private_data` (Ratsit data is NOT fetched)
 */
...
class HittaRatsitScraper {
  ...
  async saveRatsitToDatabase(ratsitData) {
    ...
    try {
      ...
      const [existingRows] = await db.execute(
        'SELECT id FROM ratsit_data WHERE personnummer = ? AND gatuadress = ? AND postnummer = ? LIMIT 1',
  ...
  while (hasMorePages) {
        ...
        try {
          await page.waitForSelector('li[data-test="person-item"]', { timeout: 15000 });
        } catch (error) {
        ...
        for (let i = 0; i < pageTotal; i++) {
          ...
          if (personData) {
            ...
            // Do NOT queue Ratsit processing — we only use Hitta data in this script.
          } else {
        ...
        // STEP 2: Upsert private_data for all saved Hitta records (no Ratsit data)
        if (pendingToSave.length > 0) {
          console.log(`\n→ [STEP 2] Upserting ${pendingToSave.length} record(s) to private_data (Hitta only)...`);
          for (const p of pendingToSave) {
            try {
              await this.saveToPrivateData(p, null);
            } catch (e) {
              console.log(`  ✗ private_data upsert error for ${p.personnamn}: ${e.message}`);
            }
          }
        }
  ...
  // Ratsit processing removed: this script only scrapes Hitta.se and upserts to hitta_se and private_data

  // markHittaAsRatsitProcessed removed (no Ratsit processing)
...
async function main() {
  program
    .description('Scrape person data from hitta.se and save to hitta_se and private_data (no Ratsit)')
