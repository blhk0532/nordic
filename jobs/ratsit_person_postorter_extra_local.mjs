...
async function getDatabaseConnection() {
    ...
    const dbPassword = process.env.DB_PASSWORD || "bkkbkk";
    const dbName = process.env.DB_DATABASE || "nuno";
    const dbCharset = process.env.DB_CHARSET || "utf8mb4";
    const dbSocket =
        process.env.DB_SOCKET || process.env.DB_UNIX_SOCKET || null;
...
async function scrapeRatsitPostorter(url, connection) {
    if (!url) {
    ...
    try {
        ...
        // Determine kommun from URL path if available
        ...
        try {
            ...
            if (pIndex !== -1 && parts.length > pIndex + 1) {
                ...
                if (k) {
                    kommunValue = decodeURIComponent(k)
                        .replace(/-kommun$/i, "")
                        .replace(/-/g, " ");
                }
            }
        ...
        // Connection is passed as parameter now
        ...
        for (const postort of postorter) {
            try {
                ...
                const [rows] = await connection.execute(
                    "SELECT id FROM ratsit_postorter WHERE post_ort = ? AND post_nummer = ?",
...
async function main() {
    ...
    try {
        ...
        if (url) {
            ...
            console.log(
                "No URL provided, fetching from ratsit_kommuner table...",
            );
            const kommunerLinks = await getKommunerLinks(connection);

            if (kommunerLinks.length === 0) {
                console.log("No personer_link found in ratsit_kommuner table");
                return;
            }

            console.log(`Found ${kommunerLinks.length} kommuner to process`);
