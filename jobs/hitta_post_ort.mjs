...
class HittaScraper {
  constructor(api_url, api_token) {
    this.api_url = api_url || process.env.LARAVEL_API_URL  || 'https://nordicdigitalthailand.com';
  ...
  async saveHittaToDatabase(hittaData) {
    ...
    try {
      ...
      if (is_ratsit) {
        try {
          // Check if duplicate already exists in ratsit_data (same gatuadress + personnamn)
          try {
            const checkResponse = await axios.get(`${this.api_url}/api/ratsit-data`, {
              params: {
                personnamn: hittaData.personnamn,
                gatuadress: hittaData.gatuadress,
              },
              headers: {
                'Authorization': this.api_token ? `Bearer ${this.api_token}` : undefined,
                'Content-Type': 'application/json',
            ...
            // If record exists with same personnamn + gatuadress, skip duplicate
            if (checkResponse.data && checkResponse.data.data && checkResponse.data.data.length > 0) {
              console.log(`  ℹ Skipping ratsit_data - duplicate exists: ${hittaData.personnamn} at ${hittaData.gatuadress}`);
              return true;
            }
          } catch (checkError) {
            // If check fails, continue with posting (don't block on duplicate check error)
            if (checkError.response?.status !== 404) {
              console.log(`  ℹ Could not verify duplicates in ratsit_data, proceeding with post`);
            }
          }
          ...
          await axios.post(`${this.api_url}/api/ratsit-data`, ratsitData, {
            headers: {
              'Authorization': this.api_token ? `Bearer ${this.api_token}` : undefined,
              'Content-Type': 'application/json',
            },
