...
class RatsitScraper {
   ...
   * @param {string} postort - Optional postort filter
   * @returns {Promise<Array>} Array of queued records
   */
  async fetchQueuedRecords(limit = 100000, postnummer = null, postort = null) {
    try {
      let apiUrl = `${this.api_url}/api/ratsit-data?is_queued=true&per_page=${limit}&sort_by=created_at&sort_direction=asc`;

      // Add postort filter if provided
      if (postort) {
        apiUrl += `&postort=${encodeURIComponent(postort)}`;
      }
...
async function main() {
  ...
  const postort = options.postort || null;

  const scraper = new RatsitScraper(options.apiUrl, options.apiToken);

  try {
    // Fetch queued records from database (optionally filtered by postnummer and/or postort)
    const queuedRecords = await scraper.fetchQueuedRecords(limit, postnummer, postort);
