...
class HittaScraper {
  ...
  await page.goto(pageUrl, { waitUntil: 'domcontentloaded' });
        ...
        if (currentPage === 1) {
          try {
            ...
            if (countElement) {
              const countText = await this.getText(countElement);
  ...
  let savedThisPage = 0; // records saved to DB for this page
        ...
        // When startPage is specified, only process that specific page
        const isSinglePageMode = startPage > 0;
        if (!isSinglePageMode && this.results.length >= effectiveMax) {
          console.log(`\n→ Reached max results limit (${effectiveMax}) after processing page ${currentPage}`);
          hasMorePages = false;
        }
        // When in single page mode, stop after processing the requested page
        else if (isSinglePageMode && currentPage >= startPage) {
          console.log(`\n→ Completed processing requested page ${startPage}`);
          hasMorePages = false;
        }
            ...
            try {
              ...
              if (newUrl !== currentUrl && newUrl.includes('revealNumber')) {
                ...
                for (const n of numbers) {
                  ...
                  // Normalize phone numbers to complete +46XXXXXXXXX format
                  const normalized = deduped
                    .map(phone => this.normalizePhoneNumber(phone))
                    .filter(phone => phone !== null);

                  data.telefon = normalized;
                  if (normalized.length > 0) {
                    console.log(`  → Revealed phone(s): ${normalized.join(', ')}`);
                  } else if (deduped.length > 0) {
                    console.log(`  → Warning: Found ${deduped.length} phone number(s) but all were incomplete/invalid`);
                }
                ...
                try {
                  ...
                  if (phoneMatches) {
                      const normalized = phoneMatches
                        .map(m => this.normalizePhoneNumber(m.trim()))
                        .filter(phone => phone !== null);
                      data.telefon = normalized;
                  }
                } catch {
...
async function main() {
  ...
  // Already waited for DOMContentLoaded in goto
      ...
      try {
        ...
        if (countElement) {
          const countText = await scraper.getText(countElement);
          const totalMatch = countText ? countText.match(/\d+/) : null;
          if (totalMatch) {
