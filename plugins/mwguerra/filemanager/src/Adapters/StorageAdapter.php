...
final class StorageAdapter implements FileManagerAdapterInterface
{
    ...
    public function getItems(?string $path = null): Collection
    {
        ...
        return $items->sortBy(function (StorageItem $item) {
            return ($item->isFolder() ? '0' : '1').mb_strtolower($item->getName());
        })->values();
    }
    ...
    public function getFolders(?string $path = null): Collection
    {
        ...
        return $folders->sortBy(fn (StorageItem $item) => mb_strtolower($item->getName()))->values();
    }
    ...
    public function createFolder(string $name, ?string $parentPath = null): FileManagerItemInterface|string
    {
        ...
        $newPath = $fullParent ? mb_rtrim($fullParent, '/').'/'.$name : $name;
    ...
    public function uploadFile(mixed $file, ?string $path = null): FileManagerItemInterface|string
    {
        ...
        $targetPath = $fullPath ? mb_rtrim($fullPath, '/').'/'.$originalName : $originalName;
    ...
    public function move(string $identifier, ?string $newParentPath): bool|string
    {
        ...
        $newPath = $newParent ? mb_rtrim($newParent, '/').'/'.$name : $name;
    ...
    private function normalizePath(?string $path): string
    {
        ...
        if ($this->root) {
            $normalizedRoot = mb_rtrim($this->root, '/');
     ...
     */
    private function validatePathWithinRoot(string $path): void
    {
        ...
        $afterRoot = mb_substr($normalizedPath, mb_strlen($normalizedRoot));
     ...
     */
    private function displayPath(string $fullPath): string
    {
        if ($this->root && str_starts_with($fullPath, $this->root)) {
            $path = mb_substr($fullPath, mb_strlen($this->root));
     ...
     */
    private function buildFolderTree(string $path, int $depth = 0): array
    {
     ...
     */
    private function copyDirectory(string $from, string $to, int $depth = 0): void
    {
        // Prevent stack overflow from deeply nested directories
        if ($depth >= self::MAX_RECURSION_DEPTH) {
