...
class FileSystemItem extends Model implements FileSystemItemInterface
{
    ...
    {
        return self::where('parent_id', $folderId)
    ...
    public static function getFileCountForFolder(?int $folderId): int
    {
        if ($folderId === null) {
            ...
            return self::where('type', '!=', FileSystemItemType::Folder->value)->count();
        }
    ...
    {
        $folders = self::where('type', FileSystemItemType::Folder->value)
    ...
    public function moveTo($newParent): bool|string
    {
        ...
        $existingItem = self::where('parent_id', $targetParentId)
    ...
    public function getFileCountRecursive(): int
    {
        ...
        $childFolders = self::where('parent_id', $this->id)
            ->where('type', FileSystemItemType::Folder->value)
            ->get();
    ...
    protected static function newFactory(): Factory
    {
        ...
        return Factory::factoryForModel(self::class);
    }
}
