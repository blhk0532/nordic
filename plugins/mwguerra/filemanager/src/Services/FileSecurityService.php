...
final class FileSecurityService
{
    ...
    public function isBlockedExtension(string $extension): bool
    {
        ...
        return in_array(mb_strtolower($extension), array_map('strtolower', $blocked));
    }
    ...
    public function hasDoubleExtension(string $filename): bool
    {
        ...
        foreach ($parts as $part) {
            if (in_array(mb_strtolower($part), array_map('strtolower', $dangerousExtensions))) {
                return true;
            }
        }
    ...
    public function validateMimeType(string $extension, ?string $mimeType): bool
    {
        ...
        $expectedMimes = $mimeMap[mb_strtolower($extension)] ?? null;
    ...
    public function sanitizeFilename(string $filename): string
    {
        ...
        if (config('filemanager.security.rename_uploads', false)) {
            $name = mb_substr(md5(uniqid()), 0, 8).'_'.$name;
        }
    ...
    public function truncateFilename(string $filename, int $maxLength): string
    {
        ...
        $extensionLength = $extension ? mb_strlen($extension) + 1 : 0;
    ...
    public function needsSanitization(string $extension): bool
    {
        ...
        return in_array(mb_strtolower($extension), array_map('strtolower', $sanitizeExtensions));
    }
     ...
     */
    private function getMimeTypeMap(): array
    {
        return [
            // Images
            'jpg' => ['image/jpeg'],
