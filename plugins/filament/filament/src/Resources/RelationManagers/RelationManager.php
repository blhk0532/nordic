...
class RelationManager extends Component implements HasActions, HasRenderHookScopes, HasSchemas, HasTable
{
    ...
    {
        return app(RelationManagerConfiguration::class, ['relationManager' => self::class, 'properties' => $properties]);
    }

    public static function getTabComponent(Model $ownerRecord, string $pageClass): Tab
    {
        return Tab::make(self::class::getTitle($ownerRecord, $pageClass))
            ->badge(self::class::getBadge($ownerRecord, $pageClass))
            ->badgeColor(self::class::getBadgeColor($ownerRecord, $pageClass))
            ->badgeTooltip(self::class::getBadgeTooltip($ownerRecord, $pageClass))
            ->icon(self::class::getIcon($ownerRecord, $pageClass))
            ->iconPosition(self::class::getIconPosition($ownerRecord, $pageClass));
    }
    ...
    {
        return self::$badgeTooltip;
    }

    public static function getTitle(Model $ownerRecord, string $pageClass): string
    {
        return self::$title ?? self::getRelationshipTitle();
    }
    ...
    public static function canViewForRecord(Model $ownerRecord, string $pageClass): bool
    {
        ...
        $model = $ownerRecord->{self::getRelationshipName()}()->getQuery()->getModel()::class;
    ...
    {
        return self::$modelLabel ?? self::getRecordLabel();
    }
    ...
    {
        return self::$pluralModelLabel ?? self::getPluralRecordLabel();
    }
    ...
    protected function makeTable(): Table
    {
        return $this->makeBaseRelationshipTable()
            ->when(self::getInverseRelationshipName(), fn (Table $table, ?string $inverseRelationshipName): Table => $table->inverseRelationship($inverseRelationshipName))
            ->when(self::getModelLabel(), fn (Table $table, string $modelLabel): Table => $table->modelLabel($modelLabel))
            ->when(self::getPluralModelLabel(), fn (Table $table, string $pluralModelLabel): Table => $table->pluralModelLabel($pluralModelLabel))
            ->when(self::getRecordTitleAttribute(), fn (Table $table, string $recordTitleAttribute): Table => $table->recordTitleAttribute($recordTitleAttribute))
            ->heading($this->getTableHeading() ?? self::getTitle($this->getOwnerRecord(), $this->getPageClass()));
    }
