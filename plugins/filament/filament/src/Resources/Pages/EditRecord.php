...
class EditRecord extends Page
{
    ...
    {
        return parent::shouldRegisterNavigation($parameters) && self::getResource()::canEdit($parameters['record']);
    }

    public function getBreadcrumb(): string
    {
        return self::$breadcrumb ?? __('filament-panels::resources/pages/edit-record.breadcrumb');
    }
    ...
    {
        return match (true) {
            $action instanceof CreateAction => fn (Schema $schema): Schema => self::getResource()::form($schema->columns(2)),
            $action instanceof EditAction => fn (Schema $schema): Schema => $schema->components([EmbeddedSchema::make('form')]),
            $action instanceof ViewAction => fn (Schema $schema): Schema => self::getResource()::infolist(self::getResource()::form($schema->columns(2))),
            default => null,
        };
    }
    ...
    {
        abort_unless(self::getResource()::canEdit($this->getRecord()), 403);
    }
