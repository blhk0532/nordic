...
class ListRecords extends Page implements Tables\Contracts\HasTable
{
    ...
    {
        return self::$breadcrumb ?? __('filament-panels::resources/pages/list-records.breadcrumb');
    }
    ...
    {
        return self::$title ?? self::getResource()::getTitleCasePluralModelLabel();
    }
    ...
    {
        if (filled($cluster = self::getCluster()) && $cluster::shouldRegisterSubNavigation()) {
    ...
    protected function makeTable(): Table
    {
        $table = $this->makeBaseTable()
            ...
            ->when(
                ...
                fn (Table $table, Model $parentRecord): Table => $table->modifyQueryUsing(
                    fn (Builder $query) => self::getResource()::scopeEloquentQueryToParent($query, $parentRecord),
                ),
            )
        ...
        self::getResource()::configureTable($table);
