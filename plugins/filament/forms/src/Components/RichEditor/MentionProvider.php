...
class MentionProvider
{
     ...
     */
    private array|Closure|null $extraAttributes = null;

    private ?Closure $getLabelsUsing = null;

    private ?Closure $getUrlUsing = null;

    private ?string $noItemsMessage = null;

    private ?string $noSearchResultsMessage = null;

    private ?string $searchingMessage = null;

    private ?string $searchPrompt = null;
    ...
    {
        return app(self::class, ['char' => $char]);
    }
        ...
        if ($this->getSearchResultsUsing instanceof Closure) {
            ...
            if (blank($search)) {
                ...
                $results = array_filter(
                    $items,
                    static fn (string $label): bool => str_contains(mb_strtolower($label), $searchLower),
                );
            }
        }
