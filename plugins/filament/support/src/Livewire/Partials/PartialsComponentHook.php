...
class PartialsComponentHook extends ComponentHook
{
    ...
    {
        $updatesCount = (int) ($this->storeGet('updatesCount') ?? 0);
        $callsCount = (int) ($this->storeGet('callsCount') ?? 0);

        if (($updatesCount + $callsCount) === 0) {
            return true;
        }

        return ($updatesCount + $callsCount) !== (int) ($this->storeGet('partialRendersCount') ?? 0);
    }
