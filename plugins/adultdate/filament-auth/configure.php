...
if ($formsOnly) {
    safeUnlink(__DIR__.'/src/FilamentAuthTheme.php');
    safeUnlink(__DIR__.'/src/FilamentAuthPlugin.php');
    ...
    if ($isTheme) {
        safeUnlink(__DIR__.'/src/FilamentAuthServiceProvider.php');
        safeUnlink(__DIR__.'/src/FilamentAuthPlugin.php');
        safeUnlink(__DIR__.'/src/FilamentAuth.php');
...
foreach ($files as $file) {
    replaceInFile($file, [
        ...
        'filament-auth' => $packageName,
        'filament-auth' => $packageSlug,
        'filament-auth_without_prefix' => $packageSlugWithoutPrefix,
        'FilamentAuth' => $className,
        'filament-auth' => $packageSlug,
        'migration_table_name' => titleSnake($packageSlug),
        'variable' => $variableName,
        'adultdate filament-auth' => $description,
    ]);

    match (true) {
        str_contains($file, determineSeparator('src/FilamentAuth.php')) => rename($file, determineSeparator('./src/'.$className.'.php')),
        str_contains($file, determineSeparator('src/FilamentAuthServiceProvider.php')) => rename($file, determineSeparator('./src/'.$className.'ServiceProvider.php')),
        str_contains($file, determineSeparator('src/FilamentAuthTheme.php')) => rename($file, determineSeparator('./src/'.$className.(str_ends_with($className, 'Theme') ? '.php' : 'Theme.php'))),
        str_contains($file, determineSeparator('src/FilamentAuthPlugin.php')) => rename($file, determineSeparator('./src/'.$className.'Plugin.php')),
        str_contains($file, determineSeparator('src/Facades/FilamentAuth.php')) => rename($file, determineSeparator('./src/Facades/'.$className.'.php')),
        str_contains($file, determineSeparator('src/Commands/FilamentAuthCommand.php')) => rename($file, determineSeparator('./src/Commands/'.$className.'Command.php')),
        str_contains($file, determineSeparator('src/Testing/TestsFilamentAuth.php')) => rename($file, determineSeparator('./src/Testing/Tests'.$className.'.php')),
        ...
        str_contains($file, determineSeparator('config/filament-auth.php')) => rename($file, determineSeparator('./config/'.$packageSlugWithoutPrefix.'.php')),
        str_contains($file, determineSeparator('resources/lang/en/filament-auth.php')) => rename($file, determineSeparator('./resources/lang/en/'.$packageSlugWithoutPrefix.'.php')),
...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i \\vendor\\ | findstr /v /i '.basename(__FILE__).' | findstr /r /i /M /F:/ ":author :vendor :package Adultdate filament-auth migration_table_name vendor_name vendor_slug adultdate@example.com"'));
}
...
function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('find ./* ./.github/* -name "vendor" -type d -prune \
     ...
     -o -type f -print0 | xargs -0 grep -E -r -l -i ":author|:vendor|:package|Adultdate|filament-auth|migration_table_name|vendor_name|vendor_slug|adultdate@example.com"'));
}
