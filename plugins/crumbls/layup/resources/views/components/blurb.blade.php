<div @if(!empty($data['id']))id="{{ $data['id'] }}"@endif class="{{ ($data['layout'] ?? 'top') === 'left' ? 'flex gap-4' : (($data['layout'] ?? 'top') === 'right' ? 'flex flex-row-reverse gap-4' : '') }} {{ \Crumbls\Layup\View\BaseView::visibilityClasses($data['hide_on'] ?? []) }} {{ $data['class'] ?? '' }}" style="{{ \Crumbls\Layup\View\BaseView::buildInlineStyles($data) }} @if(!empty($data['text_alignment']))text-align: {{ $data['text_alignment'] }};@endif" {!! \Crumbls\Layup\View\BaseView::animationAttributes($data) !!}>
