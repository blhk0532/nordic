...
final class IconSetManager
{
    ...
    private ?Collection $cachedIcons = null;
    ...
    public function searchIcons(string $query, ?array $allowedSets = null, ?string $setFilter = null): Collection
    {
        ...
        return $icons->filter(function ($icon) use ($query) {
            return str_contains(mb_strtolower($icon['name']), $query) ||
     ...
     * @return Collection<int, array{name: string, set: string, prefix: string}>
     */
    private function loadIcons(?array $allowedSets = null): Collection
    {
        $icons = collect();
        $sets = $this->getSets();
     ...
     * @return Collection<int, array{name: string, set: string, prefix: string}>
     */
    private function getIconsFromSet(string $setName, array $setConfig): Collection
    {
        $icons = collect();
        $prefix = $setConfig['prefix'] ?? $setName;
        $paths = $setConfig['paths'] ?? [$setConfig['path'] ?? null];
     ...
     */
    private function formatIconLabel(string $name): string
    {
