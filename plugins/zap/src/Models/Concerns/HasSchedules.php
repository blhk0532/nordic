...
trait HasSchedules
{
    ...
    public function isAvailableAt(string $date, string $startTime, string $endTime, ?Collection $schedules = null): bool
    {
        ...
        $schedules = $schedules ?? Schedule::where('schedulable_type', get_class($this))
    ...
    public function getAvailableSlots(
        ...
        $schedules = Schedule::where('schedulable_type', get_class($this))
            ->where('schedulable_id', $this->getKey())
            ->active()
            ->forDate($date)
            ->with('periods')
    ...
    protected function timePeriodsOverlap(string $start1, string $end1, string $start2, string $end2): bool
    {
        ...
        $start1 = mb_substr($start1, 0, 5);
        $end1 = mb_substr($end1, 0, 5);
        $start2 = mb_substr($start2, 0, 5);
        $end2 = mb_substr($end2, 0, 5);
