...
final class AutomationService
{
    ...
    private function evaluateConditions(Ticket $ticket, AutomationRule $rule, array $context): bool
    {
    ...
    private function executeActions(Ticket $ticket, AutomationRule $rule, array $context): void
    {
    ...
    private function resolveAssignee($assignTo, Ticket $ticket)
    {
    ...
    private function getRoundRobinAssignee(int $departmentId): ?int
    {
    ...
    private function getLeastLoadedAssignee(int $departmentId): ?int
    {
        $agents = DB::table(config('creators-ticketing.table_prefix').'department_users')
            ->where('department_id', $departmentId)
            ->where('can_reply_to_tickets', true)
    ...
    private function replacePlaceholders(string $text, Ticket $ticket, AutomationRule $rule): string
    {
        $replacements = [
