        ...
        return $table
            ->columns([
                ...
                TextColumn::make('agents')
                    ...
                    ->expandableLimitedList()
                    ->badge()
                    ->state(fn ($record) => $record->agents->map(fn ($agent) => UserNameResolver::resolve($agent))->toArray()
                    )
                    ->tooltip(fn ($record) => $record->agents->map(fn ($agent) => UserNameResolver::resolve($agent))->implode(', ')
                    )
                    ->color('success'),
