...
final class ImplicitlyBoundMethod extends BoundMethod
{
    ...
    protected static function getMethodDependencies($container, $callback, array $parameters = [])
    {
        ...
        foreach (self::getCallReflector($callback)->getParameters() as $parameter) {
            self::substituteNameBindingForCallParameter($parameter, $parameters, $paramIndex);
            self::substituteImplicitBindingForCallParameter($container, $parameter, $parameters);
    ...
    protected static function substituteNameBindingForCallParameter($parameter, array &$parameters, int &$paramIndex)
    {
        ...
        if (! is_null($className = self::getClassForDependencyInjection($parameter)) && ! $parameters[$paramIndex] instanceof $className) {
            return;
        }
    ...
    protected static function substituteImplicitBindingForCallParameter($container, $parameter, array &$parameters)
    {
        ...
        if (is_null($className = self::getClassForImplicitBinding($parameter))) {
            return;
        }
        ...
        if (array_key_exists($paramName, $parameters) && ! $parameters[$paramName] instanceof $className) {
            $parameters[$paramName] = self::getImplicitBinding($container, $className, $parameters[$paramName]);
        } elseif (array_key_exists($className, $parameters) && ! $parameters[$className] instanceof $className) {
            $parameters[$className] = self::getImplicitBinding($container, $className, $parameters[$className]);
        }
    }
    ...
    protected static function getClassForDependencyInjection($parameter)
    {
        ...
        if (self::isEnum($parameter)) {
            return null;
        }

        if (! self::implementsInterface($parameter)) {
            return $className;
        }
