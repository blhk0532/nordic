...
class SupportScriptsAndAssets extends ComponentHook
{
    ...
    public static function getUniqueBladeCompileTimeKey()
    {
        ...
        if (! isset(self::$countersByViewPath[$viewPath])) {
            self::$countersByViewPath[$viewPath] = 0;
        }

        $key = $viewPath.'-'.self::$countersByViewPath[$viewPath];

        self::$countersByViewPath[$viewPath]++;
