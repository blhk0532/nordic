...
class SupportLazyLoading extends ComponentHook
{
    ...
    public static function disableWhileTesting()
    {
        self::$disableWhileTesting = true;
    }

    public static function provide()
    {
        self::registerRouteMacro();
    ...
    public function mount($params)
    {
        ...
        if (isset($params['lazy']) && $params['lazy']) {
            $shouldBeLazy = true;
        }
        if (isset($params['lazy.bundle']) && $params['lazy.bundle']) {
            $shouldBeLazy = true;
        }
        if (isset($params['defer']) && $params['defer']) {
            $shouldBeLazy = true;
        }
        if (isset($params['defer.bundle']) && $params['defer.bundle']) {
            $shouldBeLazy = true;
        }

        if (isset($params['lazy']) && $params['lazy'] === 'on-load') {
            $isDeferred = true;
        }
        if (isset($params['lazy.bundle']) && $params['lazy.bundle'] === 'on-load') {
            $isDeferred = true;
        }
        if (isset($params['defer']) && $params['defer']) {
            $isDeferred = true;
        ...
        if ($lazyAttribute && ! $lazyDisabled) {
            $shouldBeLazy = true;
        }
        if ($deferAttribute && ! $deferDisabled) {
            $shouldBeLazy = true;
        }
        if ($deferAttribute && ! $deferDisabled) {
            $isDeferred = true;
        ...
        if ($lazyAttribute) {
            ...
            if ($attribute->bundle !== null) {
                $isolate = ! $attribute->bundle;
            }
            if ($attribute->isolate !== null) {
