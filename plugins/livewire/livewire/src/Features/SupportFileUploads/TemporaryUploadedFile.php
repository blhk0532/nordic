...
class TemporaryUploadedFile extends UploadedFile
{
    ...
    {
        return new self($filePath, FileUploadConfiguration::disk());
    }

    public static function signPath(string $path): string
    {
        return self::generateToken($path).':'.$path;
    }
    ...
    public static function extractPathFromSignedPath(string $signedPath): string|false
    {
        ...
        if (! hash_equals(self::generateToken($path), $token)) {
            return false;
        }
    ...
    public static function unserializeFromLivewireRequest($subject)
    {
        if (is_string($subject)) {
            if (str($subject)->startsWith('livewire-file:')) {
                return self::createFromLivewire(str($subject)->after('livewire-file:'));
            }
        ...
        if (is_array($subject)) {
            foreach ($subject as $key => $value) {
                $subject[$key] = self::unserializeFromLivewireRequest($value);
            }
        }
    ...
    {
        return mb_substr(hash_hmac('sha256', $path, app('encrypter')->getKey().session()->getId()), 0, 8);
    }
}
