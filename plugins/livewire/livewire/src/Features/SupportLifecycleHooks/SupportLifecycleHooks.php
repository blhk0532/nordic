...
class SupportLifecycleHooks extends ComponentHook
{
    ...
    public function callHook($name, $params = [])
    {
        ...
        if (! isset(self::$methodCache[$cacheKey])) {
            self::$methodCache[$cacheKey] = method_exists($this->component, $name);
        }
    ...
    public function callTraitHook($name, $params = [])
    {
        ...
        if (! isset(self::$traitCache[$class])) {
            self::$traitCache[$class] = class_uses_recursive($this->component);
        }
        ...
        foreach (self::$traitCache[$class] as $trait) {
            ...
            if (! isset(self::$methodCache[$cacheKey])) {
                self::$methodCache[$cacheKey] = method_exists($this->component, $method);
            }
