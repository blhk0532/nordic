...
final class SupportMorphAwareBladeCompilation extends ComponentHook
{
    ...
    public static function provide()
    {
        ...
        self::$shouldInjectConditionalMarkers = config('livewire.inject_morph_markers', true);
        self::$shouldInjectLoopMarkers = config('livewire.smart_wire_keys', true);

        if (! self::$shouldInjectConditionalMarkers && ! self::$shouldInjectLoopMarkers) {
            return;
        }
    ...
    public static function compileDirectives($template, $directives)
    {
        ...
        $openingDirectivesPattern = self::directivesPattern($openings);
        $closingDirectivesPattern = self::directivesPattern($closings);
        ...
        $excludedRanges = self::findIgnoredTagRanges($template, $ignoredTags);
        ...
        for ($i = count($matches[0]) - 1; $i >= 0; $i--) {
            ...
            if (self::isInExcludedRange($matchPosition, $excludedRanges)) {
                continue;
            }
            ...
            while (
                ...
                && ! self::hasEvenNumberOfParentheses($match[0])
            ) {
            ...
            if (preg_match($openingDirectivesPattern, $match[0])) {
                $template = self::prefixOpeningDirective($match[0], $template, $matchPosition);
            } elseif (preg_match($closingDirectivesPattern, $match[0])) {
                $template = self::suffixClosingDirective($match[0], $template, $matchPosition);
            } elseif (preg_match($loopEmptyDirectivePattern, $match[0])) {
                $template = self::suffixLoopEmptyDirective($match[0], $template, $matchPosition);
            }
        }
    ...
    protected static function prefixOpeningDirective($found, $template, $position)
    {
        ...
        if (self::$shouldInjectConditionalMarkers) {
            $prefix = '<!--[if BLOCK]><![endif]-->';
        }

        if (self::$shouldInjectLoopMarkers && self::isLoop($found)) {
        ...
        return self::replaceMatchIfNotInsideAHtmlTag($template, $position, $pattern, $found, $prefix, $suffix);
    }
    ...
    protected static function suffixClosingDirective($found, $template, $position)
    {
        ...
        if (self::$shouldInjectLoopMarkers && self::isEndLoop($found)) {
    ...
    protected static function suffixLoopEmptyDirective($found, $template, $position)
    {
        ...
        if (self::$shouldInjectLoopMarkers) {
            $prefix = '<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?>';
    ...
    protected static function directivesPattern($directives)
    {
        $directivesPattern = '('
            .collect($directives)
                ...
                ->sortBy(fn ($directive) => mb_strlen($directive), descending: true)
    ...
    protected static function findIgnoredTagRanges(string $template, array $tags): array
    {
        ...
        foreach ($tagMatches[0] as $tagMatch) {
            ...
            if (preg_match('/<('.$tagsPattern.')/i', $tag, $typeMatch)) {
                $type = mb_strtolower($typeMatch[1]);
    ...
    protected static function replaceMatchIfNotInsideAHtmlTag(string $template, int $position, string $pattern, string $found, string $prefix, string $suffix): string
    {
        ...
        $before = mb_substr($template, 0, $position);
        $after = mb_substr($template, $position);
        ...
        $after = mb_substr($after, mb_strlen($afterMatch[0][0]));
