...
final class LayoutCommand extends FileManipulationCommand
{
    ...
    protected function stubPath($stubSubDirectory = '')
    {
        ...
        if (! empty($stubSubDirectory) && str($stubSubDirectory)->startsWith('..')) {
            $stubDirectory = mb_rtrim(str($stubSubDirectory)->replaceFirst('..'.DIRECTORY_SEPARATOR, ''), DIRECTORY_SEPARATOR).'/';
        } else {
            $stubDirectory = mb_rtrim('stubs'.DIRECTORY_SEPARATOR.$stubSubDirectory, DIRECTORY_SEPARATOR).'/';
        }
