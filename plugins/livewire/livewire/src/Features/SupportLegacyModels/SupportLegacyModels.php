...
class SupportLegacyModels extends ComponentHook
{
    ...
    public static function hasRuleFor($component, $path)
    {
        ...
        $segmentRules = self::getRules($component);
    ...
    {
        return ! self::hasRuleFor($component, $path);
    }

    public static function getRules($component)
    {
        return self::$rules[$component->getId()] ??= self::processRules($component);
    }

    public static function getRulesFor($component, $key)
    {
        $rules = self::getRules($component);

        $propertyWithStarsInsteadOfNumbers = self::ruleWithNumbersReplacedByStars($key);

        return self::dataGetWithoutWildcardSupport(
            $rules,
    ...
    public static function dataGetWithoutWildcardSupport($array, $key, $default)
    {
        ...
        if (count($segments) > 0) {
            return self::dataGetWithoutWildcardSupport($value, implode('.', $segments), $default);
        }
    ...
    protected static function processRules($component)
    {
        ...
        return self::convertDotNotationToArrayNotation($rules);
    }

    protected static function convertDotNotationToArrayNotation($rules)
    {
        return self::recusivelyProcessDotNotation($rules);
    }
    ...
    protected static function recusivelyProcessDotNotation($rules)
    {
        ...
        foreach ($groupedRules as $key => $value) {
            $groupedRules[$key] = self::recusivelyProcessDotNotation($value);
        }
