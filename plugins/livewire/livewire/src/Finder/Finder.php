...
class Finder
{
    ...
    public function addComponent($name = null, $viewPath = null, $class = null): void
    {
        ...
        if ($name) {
            if ($class !== null) {
                $this->classComponents[$name] = $this->normalizeClassName($class);
            } elseif ($viewPath !== null) {
                $this->viewComponents[$name] = $viewPath;
            }
        }
    }
    ...
    public function addLocation($viewPath = null, $classNamespace = null): void
    {
        if ($classNamespace !== null) {
            $this->classLocations[] = $this->normalizeClassName($classNamespace);
        }
        if ($viewPath !== null) {
            $this->viewLocations[] = $viewPath;
        }
    }
    ...
    public function addNamespace($namespace, $viewPath = null, $classNamespace = null, $classPath = null, $classViewPath = null): void
    {
        ...
        if ($viewPath !== null) {
            $this->viewNamespaces[$namespace] = $viewPath;
        }
    }
    ...
    private function generateClassFromName($name, $classNamespaces = []): string
    {
        ...
        foreach ($classNamespaces as $classNamespace) {
            ...
            if (class_exists($indexClass)) {
                return $this->normalizeClassName($indexClass);
            }
            if (class_exists($selfNamedClass)) {
                return $this->normalizeClassName($selfNamedClass);
            }
        }
    ...
    private function generateNameFromClass($class): string
    {
        $class = str_replace(
            ['/', '\\'],
            '.',
        ...
        foreach ($classNamespaces as $classNamespace) {
            ...
            if ($fullName->startsWith($namespace)) {
                return (string) $fullName->substr(mb_strlen($namespace) + 1);
            }
        }
    ...
    {
        return mb_trim($className, '\\');
    }

    private function generateHashName(string $className): string
    {
    ...
    {
        return mb_trim(mb_trim($path, '/'), '\\');
    }

    private function normalizeLocation(string $location): string
    {
        return mb_rtrim($location, '/');
    }

    private function hasValidSingleFileComponentSource(string $filePath): bool
    {
    ...
    private function hasValidMultiFileComponentSource(string $dir, string $fileBaseName): bool
    {
