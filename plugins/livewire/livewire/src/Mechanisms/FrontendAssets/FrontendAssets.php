...
class FrontendAssets extends Mechanism
{
    ...
    {
        if (app(self::class)->hasRenderedStyles) {
            return '';
        }

        app(self::class)->hasRenderedStyles = true;

        $nonce = self::nonce($options);
    ...
    {
        if (app(self::class)->hasRenderedScripts) {
            return '';
        }

        app(self::class)->hasRenderedScripts = true;

        $debug = config('app.debug');

        $scripts = self::js($options);
    ...
    public static function js($options)
    {
        ...
        $url = url(app(self::class)->javaScriptRoute->uri);
        ...
        [$url, $assetWarning] = self::usePublishedAssetsIfAvailable($url, $manifest, $nonce);
        ...
        $extraAttributes = Utils::stringifyHtmlAttributes(
            app(self::class)->scriptTagAttributes,
        );
    ...
    public function boot()
    {
        ...
        Route::get(EndpointResolver::mapPath(csp: false), [self::class, 'maps']);
        Route::get(EndpointResolver::mapPath(csp: true), [self::class, 'cspMaps']);

        Blade::directive('livewireScripts', [self::class, 'livewireScripts']);
        Blade::directive('livewireScriptConfig', [self::class, 'livewireScriptConfig']);
        Blade::directive('livewireStyles', [self::class, 'livewireStyles']);
    ...
    protected static function usePublishedAssetsIfAvailable($url, $manifest, $nonce)
    {
        ...
        $assetUrl = $versionedConfiguredUrl
            ?? (app('livewire')->isRunningServerless()
                ? mb_rtrim(config('app.asset_url'), '/')."/vendor/livewire$versionedFileName"
