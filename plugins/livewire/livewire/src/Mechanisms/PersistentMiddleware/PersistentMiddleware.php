...
class PersistentMiddleware extends Mechanism
{
    ...
    // Register listeners during construction so we don't override the final Mechanism::boot()
    ...
    {
        self::$persistentMiddleware = Router::uniqueMiddleware(array_merge(self::$persistentMiddleware, (array) $middleware));
    }

    public function setPersistentMiddleware($middleware)
    {
        self::$persistentMiddleware = Router::uniqueMiddleware((array) $middleware);
    }
    ...
    protected function filterMiddlewareByPersistentMiddleware($middleware)
    {
        ...
        $persistentMiddleware = collect(app(self::class)->getPersistentMiddleware());
