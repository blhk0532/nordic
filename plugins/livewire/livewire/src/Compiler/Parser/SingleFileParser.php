...
final class SingleFileParser extends Parser
{
    ...
    public static function parse(Compiler $compiler, string $path): self
    {
        ...
        $scriptPortion = self::extractScriptPortion($mutableContents);
        $stylePortion = self::extractStylePortion($mutableContents);
        $globalStylePortion = self::extractGlobalStylePortion($mutableContents);
        $classPortion = self::extractClassPortion($mutableContents);
        $placeholderPortion = self::extractPlaceholderPortion($mutableContents);

        $viewPortion = mb_trim($mutableContents);
    ...
    public static function extractClassPortion(string &$contents): string
    {
        ...
        $classPortion = self::extractPattern($pattern, $contents);
    ...
    public static function extractScriptPortion(string &$contents): ?string
    {
        ...
        $viewContents = self::getViewPortion($contents);
    ...
    {
        $classContents = mb_trim($this->classPortion);
    ...
    {
        $viewContents = mb_trim($this->viewPortion);
    ...
    public function generateScriptContents(): ?string
    {
        ...
        if (preg_match($pattern, $this->scriptPortion, $matches)) {
            $scriptContents = mb_trim($matches[2]);
        }
        ...
        $scriptContents = mb_trim($scriptContents);
    ...
    {
        if ($this->globalStylePortion === null) {
            return null;
        }
    ...
    protected function cleanupCssIndentation(string $source): string
    {
        ...
        foreach ($lines as $line) {
            ...
            $indentation = mb_strlen($line) - mb_strlen(mb_ltrim($line));
        ...
        foreach ($lines as $line) {
            if (mb_trim($line) === '') {
                ...
                $cleanedLines[] = mb_substr($line, $minIndentation);
            }
        }
