...
class Parser
{
    ...
    private function stripTrailingPhpTag(string $contents): string
    {
        if (str_ends_with($contents, '?>')) {
            return mb_substr($contents, 0, -2);
        }

        return $contents;
    }

    private function ensureAnonymousClassHasReturn(string $contents): string
    {
    ...
    private function ensureAnonymousClassHasTrailingSemicolon(string $contents): string
    {
    ...
    private function injectViewMethod(string $contents, string $viewFileName): string
    {
        $pattern = '/}(\s*);/';
...
PHP
    ...
    private function injectPlaceholderMethod(string $contents, string $placeholderFileName): string
    {
        $pattern = '/}(\s*);/';
        preg_match_all($pattern, $contents, $matches, PREG_OFFSET_CAPTURE);
        $lastMatch = end($matches[0]);
...
PHP
    ...
    private function injectScriptMethod(string $contents, string $scriptFileName): string
    {
        $pattern = '/}(\s*);/';
        preg_match_all($pattern, $contents, $matches, PREG_OFFSET_CAPTURE);
        $lastMatch = end($matches[0]);
...
PHP
    ...
    private function injectStyleMethod(string $contents, string $styleFileName): string
    {
        $pattern = '/}(\s*);/';
        preg_match_all($pattern, $contents, $matches, PREG_OFFSET_CAPTURE);
        $lastMatch = end($matches[0]);
...
PHP
    ...
    private function injectGlobalStyleMethod(string $contents, string $globalStyleFileName): string
    {
        $pattern = '/}(\s*);/';
        preg_match_all($pattern, $contents, $matches, PREG_OFFSET_CAPTURE);
        $lastMatch = end($matches[0]);
...
PHP
    ...
    private function injectUseStatementsFromClassPortion(string $contents, string $classPortion): string
    {
