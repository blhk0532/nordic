...
final class MultiFileParser extends Parser
{
    ...
    public static function parse(Compiler $compiler, string $path): self
    {
        ...
        $placeholderPortion = self::extractPlaceholderPortion($viewPortion);
    ...
    {
        return mb_trim($this->viewPortion);
    }

    public function generateScriptContents(): ?string
    {
        // Return null if there's no script content
        if ($this->scriptPortion === null || mb_trim($this->scriptPortion) === '') {
            return null;
        }

        $scriptContents = mb_trim($this->scriptPortion);
    ...
    {
        if ($this->stylePortion === null || mb_trim($this->stylePortion) === '') {
            return null;
        }

        return mb_trim($this->stylePortion);
    }

    public function generateGlobalStyleContents(): ?string
    {
        if ($this->globalStylePortion === null || mb_trim($this->globalStylePortion) === '') {
            return null;
        }

        return mb_trim($this->globalStylePortion);
    }
    ...
    public function generateContentsForSingleFile(): string
    {
        ...
        $sfcContents = $classContents."\n\n".mb_trim($this->viewPortion);

        // Add script section if present
        if ($this->scriptPortion !== null && mb_trim($this->scriptPortion) !== '') {
            $indentedScript = $this->addJavaScriptIndentation(mb_trim($this->scriptPortion));
            $sfcContents .= "\n\n<script>\n".$indentedScript."\n</script>";
        }

        // Add style section if present
        if ($this->stylePortion !== null && mb_trim($this->stylePortion) !== '') {
            $indentedStyle = $this->addCssIndentation(mb_trim($this->stylePortion));
            $sfcContents .= "\n\n<style>\n".$indentedStyle."\n</style>";
        }

        // Add global style section if present
        if ($this->globalStylePortion !== null && mb_trim($this->globalStylePortion) !== '') {
            $indentedGlobalStyle = $this->addCssIndentation(mb_trim($this->globalStylePortion));
            $sfcContents .= "\n\n<style global>\n".$indentedGlobalStyle."\n</style>";
        }
