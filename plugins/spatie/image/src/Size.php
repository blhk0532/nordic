...
class Size
{
    ...
    public function resizeWidth(
        ...
        if ($preserveAspect) {
            $calculatedHeight = max(1, (int) round($newWidth / (new self($originalWidth, $originalHeight))->aspectRatio()));
    ...
    public function resizeHeight(?int $desiredHeight = null, array $constraints = []): self
    {
        ...
        if ($preserveAspect) {
            $calculatedWidth = max(1, (int) round($newHeight * (new self($originalWidth, $originalHeight))->aspectRatio()));
    ...
    public function relativePosition(self $size): Point
    {
