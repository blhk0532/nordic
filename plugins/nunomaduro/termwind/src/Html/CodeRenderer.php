...
class CodeRenderer
{
    ...
    public function toElement(Node $node): Element
    {
        ...
        if ($extraSpaces !== '') {
            $lines = array_map(static function (string $line) use ($extraSpaces): string {
                return str_starts_with($line, $extraSpaces) ? mb_substr($line, mb_strlen($extraSpaces)) : $line;
            }, $lines);
            $html = implode("\n", $lines);
        }

        $tokenLines = $this->getHighlightedLines(mb_trim($html, "\n"), $startLine);
        $lines = $this->colorLines($tokenLines);
        $lines = $this->lineNumbers($lines, $line);

        return Termwind::div(mb_trim($lines, "\n"));
    }
    ...
    private function lineNumbers(array $lines, int $markLine): string
    {
        ...
        $lineLength = mb_strlen((string) ($lastLine + 1));
    ...
    private function coloredLineNumber(string $token, int $lineNumber, int $length): string
    {
        return $this->styleToken(
            $token, mb_str_pad((string) ($lineNumber + 1), $length, ' ', STR_PAD_LEFT)
        );
    }
