...
class TestResult
{
    ...
    private function __construct(string $id, string $testCaseName, string $description, string $type, string $icon, string $compactIcon, string $color, string $compactColor, array $context, ?Throwable $throwable = null)
    {
        ...
        $asWarning = $this->type === self::WARN
            || $this->type === self::RISKY
            || $this->type === self::SKIPPED
            || $this->type === self::DEPRECATED
            || $this->type === self::NOTICE
            || $this->type === self::INCOMPLETE;

        if ($throwable instanceof Throwable && $asWarning) {
            if (in_array($this->type, [self::DEPRECATED, self::NOTICE])) {
                foreach (explode("\n", $throwable->stackTrace()) as $line) {
                    if (mb_strpos($line, 'vendor/nunomaduro/collision') === false) {
            ...
            $this->warning .= mb_trim((string) preg_replace("/\r|\n/", ' ', $throwable->message()));
