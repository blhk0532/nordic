...
class Writer
{
    ...
    private function renderTitleAndDescription(Inspector $inspector): self
    {
        ...
        $exception = $inspector->getException();
        $message = mb_rtrim($exception->getMessage());
    ...
    private function renderSolution(Inspector $inspector): self
    {
        ...
        foreach ($solutions as $solution) {
            ...
            $description = mb_trim((string) preg_replace("/\n/", "\n    ", $description));
    ...
    private function renderTrace(array $frames): self
    {
        ...
        foreach ($frames as $i => $frame) {
            if ($this->output->getVerbosity() < OutputInterface::VERBOSITY_VERBOSE && mb_strpos($frame->getFile(), '/vendor/') !== false) {
                $vendorFrames++;
            ...
            $pos = mb_str_pad((string) ((int) $i + 1), 4, ' ');
