                        ...
                        ->schema([
                            ...
                            Group::make([
                                TextInput::make('google_analytics_id')
                                    ...
                                    ->helperText('Your GA4 Measurement ID. Find it in GA4 > Admin > Data Streams. This enables visitor tracking on your site.'),

                                \Filament\Schemas\Components\Section::make('Dashboard Analytics (Advanced)')
                                    ->description('Optional: Display analytics data directly in the TallCMS dashboard.')
                                    ->collapsed()
                                    ->schema([
                                        \Filament\Forms\Components\Placeholder::make('setup_guide')
                                            ->label('Setup Guide')
                                            ->content(new \Illuminate\Support\HtmlString('
                                                <ol class="list-decimal list-inside space-y-2 text-sm text-gray-600 dark:text-gray-400">
                                                    <li><strong>Google Cloud Console:</strong> Go to <a href="https://console.cloud.google.com" target="_blank" class="text-primary-600 hover:underline">console.cloud.google.com</a> and create or select a project</li>
                                                    <li><strong>Enable API:</strong> Search for "Google Analytics Data API" in the API Library and enable it</li>
                                                    <li><strong>Create Service Account:</strong> Go to IAM & Admin → Service Accounts → Create Service Account (give it any name)</li>
                                                    <li><strong>Download JSON Key:</strong> Click the new service account → Keys tab → Add Key → Create new key → JSON → Download</li>
                                                    <li><strong>Grant GA4 Access:</strong> In <a href="https://analytics.google.com" target="_blank" class="text-primary-600 hover:underline">Google Analytics</a> → Admin → Property Access Management → Add the service account email (ends with @...iam.gserviceaccount.com) with "Viewer" role</li>
                                                    <li><strong>Get Property ID:</strong> In GA4 Admin → Property Settings → Copy the numeric Property ID (e.g., 123456789)</li>
                                                </ol>
                                            ')),
                                        ...
                                        TextInput::make('ga4_property_id')
                                            ...
                                            ->helperText('The numeric ID from GA4 Admin → Property Settings (not the Measurement ID)'),

                                        Textarea::make('ga4_credentials_json')
                                            ->label('Service Account JSON Key')
                                            ->rows(8)
                                            ->placeholder('Paste the entire contents of the downloaded JSON key file here...')
                                            ->helperText('The full JSON content from the key file you downloaded in step 4'),
                                    ]),
                            ])
